...
// CelestraCloud
//
// MistKit extensions for CelestraKit's Feed model
...
extension Feed {
  ...
  func toFieldsDict() -> [String: FieldValue] {
    ...
    if let lastVerified = lastVerified {
      fields["lastVerified"] = .date(lastVerified)
    }
  ...
  init(from record: RecordInfo) {
    ...
    // Date fields
    let addedAt: Date
    if case .date(let value) = record.fields["addedAt"] {
      addedAt = value
    } else {
      addedAt = Date()
    }
    ...
    if case .date(let value) = record.fields["lastVerified"] {
      lastVerified = value
    } else {
      lastVerified = nil
    }
