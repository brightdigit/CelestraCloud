...
struct FeedMistKitTests {
    ...
    func testToFieldsDictRequiredFields() {
        let feed = Feed(
            ...
            qualityScore: 75,
            subscriberCount: 100,
            addedAt: Date(timeIntervalSince1970: 1000000),
            lastVerified: nil,
            updateFrequency: nil,
            tags: [],
            totalAttempts: 5,
        ...
        // Check date field
        #expect(fields["addedAt"] == .date(Date(timeIntervalSince1970: 1000000)))
    }
    ...
    func testToFieldsDictOptionalFields() {
        let feed = Feed(
            ...
            subscriberCount: 0,
            addedAt: Date(),
            lastVerified: Date(timeIntervalSince1970: 2000000),
            updateFrequency: 3600.0,
            tags: ["tech", "news"],
            totalAttempts: 0,
            successfulAttempts: 0,
            lastAttempted: Date(timeIntervalSince1970: 3000000),
            isActive: true,
            etag: "abc123",
            lastModified: "Mon, 01 Jan 2024 00:00:00 GMT",
            failureCount: 0,
            lastFailureReason: "Network error",
            minUpdateInterval: 1800.0
        )
        ...
        #expect(fields["lastVerified"] == .date(Date(timeIntervalSince1970: 2000000)))
        #expect(fields["lastAttempted"] == .date(Date(timeIntervalSince1970: 3000000)))

        // Check optional numeric fields
        #expect(fields["updateFrequency"] == .double(3600.0))
        #expect(fields["minUpdateInterval"] == .double(1800.0))
    ...
    func testInitFromRecordAllFields() {
        let fields: [String: FieldValue] = [
            ...
            "addedAt": .date(Date(timeIntervalSince1970: 1000000)),
            "lastVerified": .date(Date(timeIntervalSince1970: 2000000)),
            "updateFrequency": .double(3600.0),
            "tags": .list([.string("tech"), .string("news")]),
            "totalAttempts": .int64(10),
            "successfulAttempts": .int64(8),
            "lastAttempted": .date(Date(timeIntervalSince1970: 3000000)),
            "etag": .string("etag123"),
            "lastModified": .string("Mon, 01 Jan 2024 00:00:00 GMT"),
            "failureCount": .int64(2),
            "lastFailureReason": .string("Timeout"),
            "minUpdateInterval": .double(1800.0)
        ]
        ...
        #expect(feed.addedAt == Date(timeIntervalSince1970: 1000000))
        #expect(feed.lastVerified == Date(timeIntervalSince1970: 2000000))
        #expect(feed.updateFrequency == 3600.0)
        #expect(feed.tags == ["tech", "news"])
        #expect(feed.totalAttempts == 10)
        #expect(feed.successfulAttempts == 8)
        #expect(feed.lastAttempted == Date(timeIntervalSince1970: 3000000))
        ...
        #expect(feed.minUpdateInterval == 1800.0)
    }
